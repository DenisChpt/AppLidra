//-----------------------------------------------------------------------
// <copiright file="Expense.cs">
//      Copyright (c) 2024 Damache Kamil, Ziani Racim, Chaput Denis. All rights reserved.
// </copyright>
// <author> Damache Kamil, Ziani Racim, Chaput Denis </author>
//-----------------------------------------------------------------------

namespace AppLidra.Shared.Models
{
    /// <summary>
    /// Represents an expense with details such as ID, name, amount, date, project ID, shares, and author.
    /// </summary>
    public class Expense
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="Expense"/> class.
        /// </summary>
        /// <param name="id">The ID of the expense.</param>
        /// <param name="name">The name of the expense.</param>
        /// <param name="amount">The amount of the expense.</param>
        /// <param name="date">The date of the expense.</param>
        /// <param name="projectId">The project ID associated with the expense.</param>
        /// <param name="shares">The list of expense shares.</param>
        /// <param name="author">The author of the expense.</param>
        /// <exception cref="ArgumentNullException">Thrown when shares is null.</exception>
        /// <exception cref="ArgumentException">Thrown when shares do not add up to 1.</exception>
        public Expense(int id, string name, double amount, DateTime date, int projectId, List<ExpenseShare> shares, string author)
        {
            Id = id;
            Author = author;
            Name = name;
            Amount = amount;
            Date = date;
            ProjectId = projectId;
            double sharesCount = 0;
            if (shares is null)
            {
                throw new ArgumentNullException(nameof(shares));
            }

            for (int i = 0; i < shares.Count; i++)
            {
                sharesCount += shares[i].Share;
            }

            if (sharesCount != 1)
            {
                throw new ArgumentException("Shares do not add up to 1");
            }

            Shares = shares;
        }

        /// <summary>
        /// Gets or sets the ID of the expense.
        /// </summary>
        public int Id { get; set; }

        /// <summary>
        /// Gets or sets the author of the expense.
        /// </summary>
        public string Author { get; set; }

        /// <summary>
        /// Gets or sets the name of the expense.
        /// </summary>
        public string Name { get; set; } = "New Expense";

        /// <summary>
        /// Gets or sets the amount of the expense.
        /// </summary>
        public double Amount { get; set; }

        /// <summary>
        /// Gets or sets the date of the expense.
        /// </summary>
        public DateTime Date { get; set; }

        /// <summary>
        /// Gets or sets the project ID associated with the expense.
        /// </summary>
        public int ProjectId { get; set; }

        /// <summary>
        /// Gets or sets the list of expense shares.
        /// </summary>
        public List<ExpenseShare> Shares { get; set; } = [];
    }
}
